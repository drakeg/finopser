...
def generate_recommendations(actor=None, today=None):
    ...
    for row, prior, growth, monthly_excess in _cost_growth_candidates(today):
        ...
        seen.add(source_key)
        recommendation, created = _upsert(
            source_key,
            {
    ...
    for resource in CloudResource.objects.filter(is_active=True, tags={}).select_related("cloud_account"):
    ...
    for violation in PolicyViolation.objects.filter(status=PolicyViolation.Status.OPEN).select_related(
        "policy", "resource", "cloud_account"
    ):
        ...
        recommendation, created = _upsert(
            source_key,
            {
                ...
                "estimated_monthly_savings": None,
                "evidence": {"policy": violation.policy.code, **(violation.evidence or {})},
    ...
    resolved = 0
    for recommendation in Recommendation.objects.filter(status=Recommendation.Status.OPEN).exclude(source_key__in=seen):
    ...
    run.open_count = Recommendation.objects.filter(status=Recommendation.Status.OPEN).count()
    run.dismissed_count = Recommendation.objects.filter(status=Recommendation.Status.DISMISSED).count()
    run.save()
